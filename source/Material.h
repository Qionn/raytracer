...
#pragma region Material LAMBERT
	...
	public:
		...
		ColorRGB Shade(const HitRecord& hitRecord, const Vector3&, const Vector3& v) override
		{
	...
	private:
		...
		float m_DiffuseReflectance	= 1.0f;
	};
#pragma endregion
...
#pragma region Material LAMBERT PHONG
	...
	private:
		ColorRGB m_DiffuseColor		= colors::White;
		float m_DiffuseReflectance	= 0.5f;
		float m_SpecularReflectance	= 0.5f;
		float m_PhongExponent		= 1.0f;
	};
#pragma endregion
...
#pragma region Material COOK TORRENCE
	...
	public:
		...
		ColorRGB Shade(const HitRecord& hitRecord, const Vector3& l, const Vector3& v) override
		{
			...
			const ColorRGB spec	= (d * f * g) / (4.0f * Vector3::Dot(v, hitRecord.normal) * Vector3::Dot(l, hitRecord.normal));
			const ColorRGB kd	= (m_Metalness == 0.0f) ? 1.0f - f : colors::Black;

			return BRDF::Lambert(kd, m_Albedo) + spec;
		}
